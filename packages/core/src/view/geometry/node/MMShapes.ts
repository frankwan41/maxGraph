...
class ParallelogramShape extends ActorShape {
    ...
    redrawPath(c: AbstractCanvas2D, x: number, y: number, w: number, h: number): void {
        const dx = w * Math.max(0, Math.min(1, parseFloat(getValue(this.style, 'startSize', this.startSize))));
...
class NegativeShape extends ActorShape {
    ...
    redrawPath(c: AbstractCanvas2D, x: number, y: number, w: number, h: number): void {
        ...
        c.curveTo(w, 0.45 * h, w - 0.4 * width, 0.25 * h, w - 0.7 * width, 0.13 * h);
...
class PersonShape extends ActorShape {
    ...
    redrawPath(c: AbstractCanvas2D, x: number, y: number, w: number, h: number): void {
        ...
        // head
        c.moveTo(w / 2, h / 6);
        c.curveTo(w / 2 - 0.1 * h, h / 6, w / 2 - 0.1 * h, 0.02 * h, w / 2, 0.02 * h);
...
export {ParallelogramShape, HeartShape, NegativeShape, PersonShape, MMCloudShape};
